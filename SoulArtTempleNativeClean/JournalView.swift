        ...
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                Button {
                    ...
                    if !entry.isEmpty {
                        if stage == "oracle",
                           let oracleIndex = discoveryStore.sessions.firstIndex(where: { $0.type == "oracle" }) {
                            // 🔮 Append personal reflection to oracle session
                            let existing = discoveryStore.sessions[oracleIndex].reflection ?? ""
                            discoveryStore.sessions[oracleIndex].reflection = existing + "\n\n📝 My reflection:\n" + entry
                            discoveryStore.saveSessions()
                        } else if let releaseIndex = discoveryStore.sessions.firstIndex(where: { $0.type == "release" && $0.reflection == nil }) {
                            // 🔁 Attach to most recent release session without a reflection
                            discoveryStore.sessions[releaseIndex].reflection = entry
                            discoveryStore.saveSessions()
                        } else {
                            // 🆕 Standalone journal — create new session
            ...
            .onAppear {
                if entry.isEmpty && !initialEntry.isEmpty { entry = initialEntry }
                if music.currentTrack == nil { music.playTrack("432hz") }
